...
const SESSION_KEY = "saloom-user-session";
...
const isValidUser = (user: AuthSession["user"]) => {
  return user.roles?.some((role) => role.toLowerCase() === "user");
};
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const persistSession = useCallback(async (value: AuthSession | null) => {
    ...
    try {
      ...
      console.warn("No se pudo persistir la sesión del usuario:", err);
    }
  }, []);
  ...
  const login = useCallback(
    async ({ email, password }: LoginParams) => {
      ...
      if (!isValidUser(payload.user)) {
        throw new Error(
          "Solo los usuarios registrados pueden acceder a esta app. Contacta al administrador."
        );
      }
